...
app.get("/", (req, res) => res.send("Airtable Clone API"));
...
app.delete("/api/tables/:id", auth, (req, res) => {
  db.query("DELETE FROM tables WHERE id = ? AND user_id = ?", [req.params.id, req.user.id]);
...
app.put("/api/records/:id", auth, (req, res) => {
  db.query("UPDATE records SET data = ? WHERE id = ?", [JSON.stringify(req.body.data), req.params.id]);
